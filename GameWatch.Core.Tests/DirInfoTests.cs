using System.Threading.Tasks;
using GameWatch.Core.Tests.Mocks;

namespace GameWatch.Core.Tests;

public sealed class DirInfoTests
{
  [Test]
  public async Task ConstructDirInfo()
  {
    var dirSeparator = DirInfo.GetDirSeparator(System.IO.Path.DirectorySeparatorChar);
    const string folderLevel1 = "folder1";
    const string folderLevel2 = "folder2";
    const string folderLevel3 = "folder3";

    var dirInfo = new DirInfo(folderLevel1).ToChild(folderLevel2).ToChild(folderLevel3);
    var dirInfo2 = new DirInfo().ToChild(folderLevel1);
    var dirInfo3 = new DirInfo();
    var dirInfo4 = new DirInfo(dirInfo3);
    var dirInfo5 = new DirInfo();

    dirInfo3 = dirInfo3.ToChild(folderLevel2);

    await Assert.That(dirInfo.Path()).IsEqualTo(dirSeparator + folderLevel1 + dirSeparator + folderLevel2 + dirSeparator + folderLevel3);
    await Assert.That(dirInfo2.Path()).IsEqualTo(dirSeparator + folderLevel1);
    await Assert.That(dirInfo3.Path()).IsEqualTo(dirSeparator + folderLevel2);
    await Assert.That(dirInfo4.Path()).IsEqualTo(dirSeparator);
    await Assert.That(dirInfo5.Path()).IsEqualTo(dirSeparator);
  }

  [Test]
  public async Task GetAllComponents()
  {
    var dirSeparator = DirInfo.GetDirSeparator(System.IO.Path.DirectorySeparatorChar);
    const string folderLevel1 = "folder1";
    const string folderLevel2 = "folder2";
    const string folderLevel3 = "folder3";

    var dirInfo = new DirInfo(folderLevel1).ToChild(folderLevel2).ToChild(folderLevel3);

    await Assert.That(dirInfo.Path()).IsEqualTo(dirSeparator + folderLevel1 + dirSeparator + folderLevel2 + dirSeparator + folderLevel3);
    await Assert.That(dirInfo.ToParent().Path()).IsEqualTo(dirSeparator + folderLevel1 + dirSeparator + folderLevel2);
    await Assert.That(dirInfo.Stem()).IsEqualTo(folderLevel3);
  }

  [Test]
  public async Task ChangingFolderLevels()
  {
    var dirSeparator = DirInfo.GetDirSeparator(System.IO.Path.DirectorySeparatorChar);
    const string folderLevel1 = "folder1";
    const string folderLevel2 = "folder2";
    const string folderLevel3 = "folder3";
    const string folderLevel4 = "folder4";

    var dirInfo = new DirInfo(folderLevel1).ToChild(folderLevel2).ToChild(folderLevel3).ToChild(folderLevel4);
    var dirInfo2 = new DirInfo(folderLevel1).ToChild(folderLevel2).ToChild(folderLevel3).ToChild(folderLevel4);

    await Assert.That(dirInfo.ToParent().ToParent().Path()).IsEqualTo(dirSeparator + folderLevel1 + dirSeparator + folderLevel2);
    await Assert.That(dirInfo2.ToParent().ToParent().ToChild(folderLevel4).Path()).IsEqualTo(dirSeparator + folderLevel1 + dirSeparator + folderLevel2 + dirSeparator + folderLevel4);
  }

  [Test]
  public async Task PathsRelativeToCurrentFolder()
  {
    var dirSeparator = DirInfo.GetDirSeparator(System.IO.Path.DirectorySeparatorChar);
    const string targetSubFolder = "folder1";
    const string targetSubFolder2 = "folder2";

    var dirInfo = new DirInfo(targetSubFolder, root: DirInfo.RootType.Root);
    var dirInfo2 = new DirInfo([targetSubFolder, targetSubFolder2], root: DirInfo.RootType.CurrentDir);

    await Assert.That(dirInfo.Path()).IsEqualTo(dirSeparator + targetSubFolder);
    await Assert.That(dirInfo2.Path()).IsEqualTo("." + dirSeparator + targetSubFolder + dirSeparator + targetSubFolder2);
  }
}
...
public sealed class MainMenu : IScene
{
    public IScene? Execute()
    {
        ...
        menu.AddOption(new MenuOption(displayText: _lang.ActiveLanguagePack.MainMenu_SettingsMenu_DisplayText, action: () => { nextScene = new SettingsMenu(lang: _lang, logger: _logger, appState: _appState, gameLibrary: _gameLibrary, appSettings: _appSettings); }));
